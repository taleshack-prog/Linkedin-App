"""baseline — schema completo do LinkPost AI (estado pós-migração manual 005)

Bancos NOVOS: `alembic upgrade head` cria tudo.
Bancos EXISTENTES (produção/local com schema aplicado): `alembic stamp head`
uma única vez — marca como migrado sem executar nada.
"""
...
def downgrade() -> None:
    op.execute(
        """
        DROP TABLE IF EXISTS brand_profiles, publish_logs, posts,
                             content_briefs, linkedin_accounts, users CASCADE;
